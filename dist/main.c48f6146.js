...
fetch("https://rickandmortyapi.com/api/character").then(function (charactereAPI) {
  console.log("Juste apr\xE8s r\xE9ception de ma r\xE9ponse : ", charactereAPI); // console.log(`Je traduis ma réponse :`, responseAPI.json());
  ...
  var personnage = charactereEnJson.data;

  for (var cpt = 0; cpt < personnage; cpt++) {
...
function addNewCard(user) {
  ...
  mainContainer.innerHTML += "<div class=\"personnage\">\n    <span>user.nom</span>\n    <span>user.status esp\xE8ce</span\n    <span>user.type</span>\n    <span> user.genre</span>\n    <span>user. plan\xE8te d'origine</span>\n    <span> user.dernier lieux de positionnement connu</span>\n    <span>  user. les \xE9pisodes dans le(s)quel(s) le personnage apparait</span>\n    </div>";
}
